...
func InviteContact(ctx context.Context, dg *diago.Diago, recipient sip.Uri, destination, transport string, opts diago.InviteOptions) (*diago.DialogClientSession, error) {
	if transport == "" {
...
func (d *DiagoDialer) Invite(ctx context.Context, recipient sip.Uri, opts diago.InviteOptions) (*diago.DialogClientSession, error) {
	return InviteContact(ctx, d.DG, recipient, recipient.HostPort(), opts.Transport, opts)
}
...
func (d *ContactDialer) Invite(ctx context.Context, recipient sip.Uri, opts diago.InviteOptions) (*diago.DialogClientSession, error) {
	...
	transport := opts.Transport
	return InviteContact(ctx, d.DG, recipient, dest, transport, opts)
}

func (d *ContactDialer) InviteRegistered(ctx context.Context, recipient sip.Uri, destination, transport string, opts diago.InviteOptions) (*diago.DialogClientSession, error) {
	return InviteContact(ctx, d.DG, recipient, destination, transport, opts)
}

// InviteExtension dials a registered extension using stored contact routing.
func InviteExtension(ctx context.Context, dg Dialer, reg *registrar.Registrar, ext string, inviteOpts diago.InviteOptions) (*diago.DialogClientSession, error) {
...
func InviteOutLeg(ctx context.Context, dg Dialer, outURI sip.Uri, opts ConnectOpts, inviteOpts diago.InviteOptions) (*diago.DialogClientSession, error) {
	dest := opts.DialDestination
	transport := opts.DialTransport
	if dest != "" {
		if cd, ok := dg.(*ContactDialer); ok {
			return cd.InviteRegistered(ctx, outURI, dest, transport, inviteOpts)
		}
		if d, ok := dg.(*diago.Diago); ok {
			return InviteContact(ctx, d, outURI, dest, transport, inviteOpts)
		}
	}
