...
type DialogClientSession struct {
	*sipgo.DialogClientSession
	...
	userReleasedHold bool // set by Unhold(); reject server sendonly re-hold (dial-tone reassert)
...
func (d *DialogClientSession) handleReInvite(req *sip.Request, tx sip.ServerTransaction) error {
	...
	d.mu.Lock()
	released := d.userReleasedHold
	d.mu.Unlock()
	if released && req.Body() != nil {
		if dir := sdpOfferDirection(req.Body()); dir == sdp.ModeSendonly || dir == sdp.ModeInactive {
...
// ReInviteMediaMode sends an in-dialog re-INVITE with the given SDP direction.
// Used to simulate real-phone SDP churn (e.g. sendrecv refresh during hold setup).
func (d *DialogClientSession) ReInviteMediaMode(ctx context.Context, mode string) error {
	m := d.MediaSession().Fork()
	m.Mode = mode
	return d.reInviteMediaSession(ctx, m)
}
