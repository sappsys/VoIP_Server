...
type MediaConfig struct {
	...
	// Experimental
	SecureRTPAlg uint16
	// RTPNAT enables symmetric RTP (learn remote source from inbound packets).
	// Set to media.RTPNATSymetric (1) for phones behind NAT.
	RTPNAT int
	// Used internally
...
func (conf *MediaConfig) update(codecs []media.Codec, rtpNAT int) {
	...
	if rtpNAT != 0 {
		conf.rtpNAT = rtpNAT
	} else if conf.RTPNAT != 0 {
		conf.rtpNAT = conf.RTPNAT
	}
}
