...
func (s *MediaSession) Fork() *MediaSession {
	...
	// Preserve symmetric-RTP learned endpoints across re-INVITE forks.
	if a := s.learnedRTPFrom.Load(); a != nil {
		cp.learnedRTPFrom.Store(a)
	}
	if a := s.learnedRTCPFrom.Load(); a != nil {
		cp.learnedRTCPFrom.Store(a)
	}
	return &cp
}
...
// SetNegotiatedMode overrides the direction used for ReadRTP/WriteRTP after an offer/answer.
// Used when phone hold answers recvonly but the server must inject dial tone without a
// racing follow-up re-INVITE.
func (s *MediaSession) SetNegotiatedMode(mode string) {
	if s == nil || mode == "" {
		return
	}
	s.mode = mode
...
// RestorePrimaryCodec moves the given codec to the front of filterCodecs so bridging
// resumes on the same codec as before hold/unhold SDP churn.
func (s *MediaSession) RestorePrimaryCodec(c Codec) {
	if s == nil || c.Name == "" {
		return
	}
	common := s.filterCodecs
	if len(common) == 0 {
		common = s.Codecs
	}
	filter := make([]Codec, 0, len(common))
	for _, x := range common {
		if x.PayloadType == c.PayloadType && x.Name == c.Name {
...
func (s *MediaSession) updateRemoteCodecs(codecs []Codec, answerer bool) int {
	...
	// Prefer local codec order on every answer (initial + re-INVITE). sessionID!=0 on
	// forked sessions made the old answerer-only check skip this, so phone hold/unhold
	// could reorder filterCodecs (e.g. G722 before PCMU) and corrupt the PCM bridge.
	if SDPCodecPreferLocalOrder > 0 {
...
func negotiateMediaDirection(remoteMode, localPref string) string {
	...
	case sdp.ModeSendrecv:
		...
		// Remote resumed bidirectional media (e.g. phone unhold). Prefer sendrecv even
		// when our forked session still carries sendonly from an earlier server Hold().
