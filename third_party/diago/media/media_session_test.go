...
func TestMediaSessionUpdateCodecPreferLocalOnReinvite(t *testing.T) {
	old := SDPCodecPreferLocalOrder
	SDPCodecPreferLocalOrder = 1
	t.Cleanup(func() { SDPCodecPreferLocalOrder = old })

	g722 := Codec{PayloadType: 9, SampleRate: 8000, Name: "G722"}
	m := &MediaSession{
		Codecs:    []Codec{CodecAudioUlaw, CodecAudioAlaw, g722},
		sessionID: 123, // forked session: was not treated as answerer before fix
	}
	m.updateRemoteCodecs([]Codec{g722, CodecAudioAlaw, CodecAudioUlaw}, false)
	...
	assert.Equal(t, g722, m.filterCodecs[2])
}

func TestMediaSessionRestorePrimaryCodec(t *testing.T) {
	g722 := Codec{PayloadType: 9, SampleRate: 8000, Name: "G722"}
	m := &MediaSession{
		filterCodecs: []Codec{g722, CodecAudioUlaw, CodecAudioAlaw},
	}
	m.RestorePrimaryCodec(CodecAudioUlaw)
	require.Len(t, m.filterCodecs, 3)
	assert.Equal(t, CodecAudioUlaw, m.filterCodecs[0])
}
